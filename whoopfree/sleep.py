...
def _motion_magnitude(row) -> int:
    ax = abs(row["accel_x"] or 0)
    ay = abs(row["accel_y"] or 0)
    az = abs(row["accel_z"] or 0)
    return ax + ay + az
...
def detect_sleep_window(samples: Sequence[sqlite3.Row], night_of: date) -> tuple[datetime, datetime] | None:
    ...
    if not hrs:
        return None
    hr_threshold = statistics.mean(hrs) * 0.95  # below average HR
    ...
    for r in samples:
        is_sleeping = (
            (r["heart_rate_bpm"] or 999) < hr_threshold
            and _motion_magnitude(r) < motion_threshold
        )
        if is_sleeping:
            cur.append(r)
        else:
...
def classify_stages(
    ...
    for ep in epochs:
        ...
        motions = [_motion_magnitude(r) for r in ep]
        epoch_stats.append(
            {
    ...
    for e in epoch_stats:
        ...
        if hr is None:
            ...
            continue
        if (motion is not None and motion > 200) or hr > hr_baseline + 12:
            raw_stages.append("wake")
        elif motion is not None and motion < 30 and hr <= hr_min + 5 and (
            rmssd is None or rmssd <= rmssd_baseline
        ):
            raw_stages.append("deep")
        elif motion is not None and motion < 60 and hr >= hr_min + 6 and (
